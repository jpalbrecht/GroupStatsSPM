...
[ySessLen, xSessLen] = size(spmStruct.Sess);
[yULen, xULen] = size(spmStruct.Sess.U);
% initiate counter
conIt = 1;
gppiContrasts = struct();
% loop over sessions
for sessIt=1:1:xSessLen
   % loop over Predictors (U) - e.g. TASKS
   for preIt=1:1:xULen
       ui = spmStruct.Sess.U(1,preIt);
       ...
       for mainIt=1:1:numMain
           ...
           for paraIt=1:1:numPara
               ...
               if pi.h >0 % else its 'none'
                   % add contrast 
                   gppiContrasts(conIt).name = [mainName '_' pi.name];
